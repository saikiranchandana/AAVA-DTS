...
def get_secret(name: str, default=""):
    # Prioritize environment variables (.env) to avoid Streamlit secrets warnings
    val = os.getenv(name)
    if val:
        return val
    try:
        # Fallback to st.secrets if available, but skip if missing to avoid logs clutter
...
WORKFLOW = {"name": "TSB_Data_Lineage_Generator_WF", "pipelineId": 2256}
...
def execute_workflow_async(pipeline_id, user_inputs_dict, user_email="samuvel.isaac@ascendion.com"):
    ...
    rr = requests.get(res_url, headers=headers, verify=False, timeout=20)
    rr.raise_for_status()
    res_data = rr.json()
    
    # Extract output: data.result.response.output
    ans = res_data.get("data", {}).get("result", {})
    ...
    if isinstance(ans, dict):
        ...
        if resp:
            if isinstance(resp, dict): output = resp.get("output", "")
            else:
                try: output = json.loads(str(resp)).get("output", str(resp))
                except: output = str(resp)
        if not output: output = ans.get("expected_output", "")
    
    # Curation: Handle markdown-like fenced CSV output
    curated = str(output).strip()
...
def process_agent(file_content: str):
    ...
    try:
        output, duration = execute_workflow_async(WORKFLOW["pipelineId"], {"{{Program_Files}}": file_content})
        # Use "Agent" as the key to match previous expectations if specific names aren't needed, 
        # or we can just name it "agent" to be generic.
        return {"success": True, "raws": {"Agent": output}, "api_duration": duration}
...
def create_dq_monitor(SOURCE_ID, row, idx):
    ...
    def get_column_value(row, *possible_names):
        """Try multiple column name variations (handles case sensitivity)"""
        for name in possible_names:
            if name in row.index and pd.notna(row[name]):
                return str(row[name]).strip()
        return None
    
    monitor_name = get_column_value(row, "Monitor Name", "monitor name", "Monitor_Name", "monitor_name") or f"Monitor {idx+1}"
    description = get_column_value(row, "Description", "description") or "No description provided."
    custom_sql = get_column_value(row, "Custom_SQL", "custom_sql", "Custom SQL", "custom sql")
    threshold_str = get_column_value(row, "Threshold", "threshold") or "0"
    
    # Validate required field
    if not custom_sql:
        return {
            "Monitor Name": monitor_name, 
            "Status": "⚠️ Skipped (empty Custom_SQL)", 
            "Response": "Custom_SQL column is required and cannot be empty"
        }
    ...
    # API requires max > min (min=0), so ensure max is at least 1
    if threshold_val <= 0:
        threshold_val = 1
    ...
    try:
        ...
        return {
            ...
            "Status": "✅ Created" if res.status_code == 200 else f"❌ Failed ({res.status_code})", 
            "Response": res.text
        }
    except Exception as e:
        return {
...
def tab3_create_dq_rules():
    ...
    if ddl_content:
        ...
        if st.button("🚀 Generate DQ Rules from DDL"):
            ...
            #         rv = t.get("raw", "")
            #         if rv: raws[nm] = rv.split("\n\n", 1)[0]

            with st.spinner("Calling AVA pipeline..."):
                try:
                    output, duration = execute_workflow_async(dq_pipeline_id, {"{{DDL_INPUT_FILE}}": ddl_content})
                    raws = {"Agent": output}
...
def tab5_create_link_mapping():
    ...
    if ddl_content and glossary_content1:
        if st.button("🚀 Generate Link Mapping"):
            ...
            # payload = {
            #     "pipeLineId": pipeline_id,
            #     "userInputs": {"{{postgres_DDL}}": ddl_content, "{{glossary_file}}": glossary_content1},
            ...
            with st.spinner(f"Running pipeline: {workflow_name}..."):
                try:
                    output, duration = execute_workflow_async(pipeline_id, {
